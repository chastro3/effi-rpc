...
public class AnnotationConfigParser<T extends AnnotatedElement, A extends Annotation> {
    ...
    public <V> AnnotationConfigParser(Class<A> type, ConfigName<V> configName, Function<A, V> valueGetter) {
        ...
        if (annotation != null) {
            if (CollectionUtil.isNotEmpty(kvMappers)) {
                for (KVMapper<A, ?> kvMapper : kvMappers) {
                    config.set(kvMapper.name().name(), kvMapper.valueGetter().apply(annotation));
                }
            }
        }
    }
