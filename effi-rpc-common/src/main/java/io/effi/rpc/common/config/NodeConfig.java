 ...
 */
public class NodeConfig extends FlatConfig implements LinkedConfig {
    ...
    public NodeConfig(Object owner) {
        this(owner, null);
    }

    public NodeConfig(Object owner, Config parent) {
        super(owner);
        if (parent != null) {
            this.parent = parent;
        }
    ...
    public String get(ConfigKey configKey) {
        ...
        String value = null;
        if (source == Source.SELF_PREFERRED) {
            value = getSelfPreferred(key);
        } else if (source == Source.PARENT_PREFERRED) {
            value = getParentPreferred(key);
        } else if (source == Source.CASCADED) {
    ...
    public List<String> getCascaded(ConfigKey configKey) {
        return configKey.source() == Source.CASCADED
    ...
    public List<String> getCascaded(String key) {
        ...
        if (parent != null) {
            if (parent instanceof NodeConfig nodeConfig) {
