...
public class FilterChain {
    ...
    public static <C extends ExecutorContext<?, ?, C>> Result execute(C context, List<? extends Filter<?, ?, ?>> filters) {
        ...
        if (context instanceof InvocationContext<?, ?> invocationContext) {
            filterContext = new FilterInvocationContext<>(invocationContext);
        } else if (context instanceof ReplyContext<?, ?> replyContext) {
            filterContext = new FilterReplyContext<>(replyContext);
        } else {
            throw new IllegalArgumentException(Messages.unSupport("context", context.getClass()));
        }
