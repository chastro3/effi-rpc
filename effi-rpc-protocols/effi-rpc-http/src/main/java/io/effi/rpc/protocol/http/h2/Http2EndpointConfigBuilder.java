...
public interface Http2EndpointConfigBuilder<T extends EndpointConfig, C extends Http2EndpointConfigBuilder<T, C>> extends EndpointConfigBuilder<T, C> {
    ...
    default C headerTableSize(int headerTableSize) {
        config().set(DefaultConfigKeys.HEADER_TABLE_SIZE, headerTableSize);
        return returnThis();
    }
    ...
    default C initialWindowSize(int initialWindowSize) {
        config().set(DefaultConfigKeys.INITIAL_WINDOW_SIZE, initialWindowSize);
        return returnThis();
    }
    ...
    default C maxConcurrentStreams(int maxConcurrentStreams) {
        config().set(DefaultConfigKeys.MAX_CONCURRENT_STREAMS, maxConcurrentStreams);
        return returnThis();
    }
    ...
    default C maxFrameSize(int maxFrameSize) {
        config().set(DefaultConfigKeys.MAX_FRAME_SIZE, maxFrameSize);
        return returnThis();
    }
    ...
    default C maxHeaderListSize(int maxHeaderListSize) {
        config().set(DefaultConfigKeys.MAX_HEADER_LIST_SIZE, maxHeaderListSize);
        return returnThis();
    }
}
