...
import static io.effi.rpc.protocol.http.h1.Http1Protocol.NAME;
...
public class Http1Protocol extends HttpProtocol {
    ...
    public <T> Servant createServant(ServantMethod<T> servantMethod, HierarchicalOptions options, ScopedModule module) {
        return Http1Servant.builder(servantMethod).options(options).module(module).build();
    }

    @Override
    public <T> Caller<T> createCaller(TypeCapture<T> returnType, HierarchicalOptions options, ScopedModule module) {
        return Http1Caller.builder(returnType).options(options).module(module).build();
    }
